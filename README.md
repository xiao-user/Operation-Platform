...
- 教育局“区域教育总览”已实现独立数字孪生首页：以真实榕城区 GeoJSON 构建 Three.js 三维区域地图，展示教育局与 43 个公开学校 POI，并支持同场景镇街聚焦下钻、平滑返回和三套主题切换；页面在新标签打开且不加载应用 Shell。
...
- 教育局智慧大脑：榕城区教育数字孪生首页，包含三维行政区地图、学校空间点位、类型统计、机构空间档案、地图悬浮、相机视角控制、镇街聚焦下钻和生态荧光/深海矩阵/城市琥珀三套主题。榕城区相邻的揭东区、普宁市、潮安区、潮阳区和金平区作为低对比度二维环境层展示；下钻时保持同一场景和投影，当前镇街提升为独立三维焦点、其余镇街压平，同级镇街仍可悬浮并点击平滑切换焦点，点击地图外层或面包屑可平滑返回区级视角。区级流光使用完整榕城区边界，下级流光跟随当前镇街边界。地图使用 `OrbitControls` 并以地图厚度方向 `Z` 轴作为朝上方向，提供左键受限三维旋转、滚轮缩放及阻尼惯性，地图平移关闭。默认折叠的调试台支持构图与操作、相机位置/焦点 XYZ、区域视角、下级聚焦、环境边界和颜色覆盖等参数即时调节，参数不持久化。页面采用全视口地图底层与顶部、左右、底部 HUD 覆盖层，地图不会因信息面板缩窄；HUD 颜色、字体、间距、尺寸、层级和动效由独立页面设计令牌统一维护。页面 HUD、地图舞台、主题配置、数据适配与 Three.js 渲染器已分层。首版点位和行政区边界来自公开地理数据，正式上线前需由教育局权威台账校准。
...
榕城区三维地图由项目内的独立渲染内核实现，使用分层 Three.js 场景、批量点位与飞线、确定性程序纹理和显式 GPU 资源生命周期。镇街边界来自 OpenStreetMap contributors，通过 Nominatim 获取并按 ODbL 使用。
