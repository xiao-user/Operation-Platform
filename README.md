...
工作台使用 12 列 Bento 网格，桌面端可进入独立编辑模式进行显隐、排序、缩放和轻量设置，并以显式“保存/取消”提交个人布局。平板自动压缩为 6 列并按两列成组填满，落单组件占满整行；手机按桌面顺序单列只读展示。学校、教育局、机构和平台组件统一使用轻边框、低装饰的内容容器；教育局管理型和业务型模板额外提供快捷应用入口、局内新闻、信息公开、教学应用排行榜、消息与待办中心、日程与任务管理、个人成长与发展、我的订阅和通知公告。教育局日程使用 Element Calendar，支持日期与月份切换、日程新增/编辑/完成/删除；待办、新闻公告、排行和订阅提供筛选、详情及状态操作。当前业务内容由类型化 Mock 数据源提供，未来替换真实 API 时不需要改变组件模板和布局模型。
