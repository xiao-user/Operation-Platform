...
- 按“组织类型 × 管理/业务角色”提供 8 套固定 Bento 组件清单；经典工作台保留自由网格拖拽与缩放，新版提供“完整流式”和“分两列”两种轻量布局，可调整顺序、半行/整行宽度或主辅列归属；两版布局独立保存，但共用组件、数据与权限。
