...
当前顶部模块、侧边菜单、默认路径和页面子导航主要写在 `src/config/navigation.ts` 与路由文件中。不同学校、教育局和机构只能共享同一类静态配置，无法由管理员针对具体租户调整。

本功能新增系统级“菜单配置”页面，让管理员按具体租户维护顶部模块和侧边菜单。第一期使用 `localStorage` 持久化，但通过仓库接口隔离存储实现，后续可以替换成后端 API。

## 2. 目标

- 支持查询、新增、编辑、删除、排序和显隐菜单。
...
3. 配置范围为顶部模块和侧边菜单。
4. 配置粒度为具体租户，不只是学校、教育局或机构类型。
5. 新租户从所属类型的默认模板复制初始配置。
6. 使用独立的系统级配置页面，通过租户选择器维护任意租户。
...
学校、教育局和机构分别保留默认菜单模板。当前 `topNavTabs`、`moduleMenus` 和默认路径迁移为这些模板的初始数据。
...
- 新增固定路由 `/system/menu-config`。
- 在顶部用户菜单中为管理员增加“菜单配置”入口。
- 该路由和入口不属于租户自定义菜单，不能通过菜单配置删除或隐藏。
...
行操作包括新增子菜单、编辑、删除。第一期使用排序数字控制同级顺序，不增加拖拽依赖。
